    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-lg-5 mb-4">
                ...
                <hr>
                <h5 class="@if ($product->stock > 0) text-dark @else text-danger @endif bold">@if ($product->stock > 0) In Stock @else Out of
                        Stock @endif
                </h5>
                <form action="{{ route('baskets.store') }}" method="post" class="d-inline">
                    <div class="d-flex align-items-left mt-3">
                        ...
                        <div class="form-group col-lg-4">
                            <input class="form-control @if($errors->count()) is-invalid @else border-text @enderror" name="quantity"
                                    type="number" min="{{ $product->minimum }}" max="{{ $product->maximum }}"
                                    value="{{ $product->minimum }}"  @if ($product->stock <=0) disabled @endif>
                        </div>
                    </div>
                    {{-- @if($errors->count()) <small class="text-danger text-sm w-100"> {{ $errors->all()[0] }} </small> @endif --}}
